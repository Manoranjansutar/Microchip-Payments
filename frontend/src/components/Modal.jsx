...
const Modal = ({ openModal, setOpenModal }) => {
 ...
 //Google map component
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:5000/api/microchip/createAddress",
        formData
      );
  ...
  const handleYourLocation = async () => {
    ...
    try {
      const response = await axios.get(
        "http://localhost:5000/api/microchip/getAddress"
      );
  ...
  const fetchSavedAddress = () => {
    const endpoint = searchQuery
      ? `http://localhost:5000/api/microchip/search?q=${encodeURIComponent(
          searchQuery
        )}`
      : "http://localhost:5000/api/microchip/getAddress";
